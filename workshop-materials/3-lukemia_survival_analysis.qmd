---
title: "Survival analysis techniques for Leukemia Remission data"
# title-slide-attributes:
#   data-background-image: images/slide-1.0.png
...
::: {layout-ncol=3}
![](images/slide-1.0.png){height=400px}

![](images/slide-1.1.png){height=400px}

![](images/slide-1.2.png){height=400px}
:::
...
library(dplyr)
data_raw <- read.xlsx(
  "lukemia-remission-data.xlsx",
  rowNames = FALSE, detectDates = TRUE,
  skipEmptyRows = TRUE, skipEmptyCols = TRUE) 
# data_raw |> count(status) # 0(no remission):12, 1(remission):30
# data_raw |> count(sex) # 0(male):22, 1(female):20
data_raw |> glimpse()
```

Remission `status` indicates whether patients achieved remission during the observation period. It refers to the degree to which the disease has responded to treatment, particularly whether cancerous cells can still be detected in the body. `time` to event (in days) is the duration until remission or censoring. `logWBC` (Log of white blood cell count) serves as a key clinical biomarker. `rx` represents treatment protocol, indicates the specific therapeutic approach implemented for each patient. 
This dataset structure enables multifaceted analysis of factors influencing leukemia remission, allowing researchers to identify both clinical and demographic predictors of patient outcomes.
...
library(survival)
library(survminer)
fit_km <- survfit(Surv(time, status) ~ 1, data = data_raw)
```
## 
...
The survival probability is at 12 time units (e.g., months) is approximately 0.5, indicating that about half of the patients are expected to survive beyond this time point. It says that we can be `95%` confident that the true median survival time is between 8 and 22 units.

## Cox proportional hazards regression
`Cox regression` enables simultaneous assessment of multiple variables' impact on leukemia remission. For our dataset, this means quantifying how `sex`, `logWBC`, and treatment protocol collectively influence hazard rates while controlling for confounding factors. This semi-parametric approach offers flexibility without requiring specification of the baseline hazard function, making it ideal for identifying significant predictors of leukemia remission.
...
## Cox proportional hazards regression - analysis II
...
data_raw |> 
  ...
  labs(title = "Risk score distribution by event status",
...
#| 
data_raw |> 
  mutate(status = ifelse(status == 1, "event", "censored")) |>
  ggplot(aes(x = risk, fill = status)) +
  geom_density(alpha = 0.5) +
  labs(title = "Density plot of risk scores by event status",
       x = "risk", fill = "status") +
  theme_minimal()
```
:::
...
#| fig-height: 8
#| 
# Create tertiles of risk
data_raw$risk_group <- with(
  data_raw, cut(risk, breaks = quantile(risk, probs = c(0, 1/3, 2/3, 1)),
                labels = c("low", "medium", "high"),
                include.lowest = TRUE))

fit_km_risk <- survfit(Surv(time, status) ~ risk_group, data = data_raw)
fit_km_risk |> 
  ggsurvplot(data = data_raw, pval = TRUE,
...
data_raw |> 
  ...
  ggplot(aes(x = logWBC, y = risk, group = sex, colour = sex)) +
  geom_point() + geom_line() +
  labs(x = "log WBC", y = "risk") +
  theme_minimal()
```
:::
